...
function App() {
  ...
  useEffect(() => {
    const onPopState = () => {
      const value = window.location.hash.replace(/^#\/?/, '') as WorkspacePage
      if (['dashboard', 'tasks', 'sessions', 'settings', 'system', 'run'].includes(value)) setPage(value)
    }
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      Promise.all([getAdminRuns(), getAdminSessions()]).then(([runs, sessions]) => {
        if (!active) return
        setAdminRuns(runs.items)
        setAdminSessions(sessions.items)
      }).catch(() => undefined)
  ...
  return <div className="app-shell">
    ...
    {showNew && <NewDiagnosis profiles={Object.keys(adminConfig?.profiles ?? { default: {} })} onClose={() => setShowNew(false)} onSubmit={startDiagnosis} />}
  </div>
}
